...
var urban=ee.FeatureCollection('users/tirthankar25/SaoPaulo');
...
//LST images
var LST_Day_AQUA_mean=ee.Image('users/tirthankar25/LST_Day_AQUA_mean_2010_vfF')
...
var LST_Night_AQUA_mean=ee.Image('users/tirthankar25/LST_Night_AQUA_mean_2010_vfF')
...
var landcover=ee.Image('users/tirthankarchakraborty/GOBLandcover1992_2015').select('b19');
...
var Diff_NDVI=NDVI_diff.reduceRegion({geometry: feature.geometry(),  reducer: ee.Reducer.mean(),  scale: 300,
});
var Diff_BSA=BSA_diff.reduceRegion({geometry: feature.geometry(),  reducer: ee.Reducer.mean(),  scale: 300,
});
var Diff_WSA=WSA_diff.reduceRegion({geometry: feature.geometry(),  reducer: ee.Reducer.mean(),  scale: 300,
});

var Diff_NDBI=NDBI_diff.reduceRegion({geometry: feature.geometry(),  reducer: ee.Reducer.mean(),  scale: 300,
});

//var Diff_EVI=EVI_diff.reduceRegion({geometry: feature.geometry(),  reducer: ee.Reducer.mean(),  scale: 300,
//});

var DEM_neigh=DEM.reduceRegion({geometry: feature.geometry(),  reducer: ee.Reducer.mean(),  scale: 300,
});

var PM_neigh=PM.reduceRegion({geometry: feature.geometry(),  reducer: ee.Reducer.mean(),  scale: 300,
});
  var dayUHI_count=dayurbanLST_AQUA.reduceRegion({geometry: feature.geometry(),  reducer: ee.Reducer.count(),  scale: 1000,
});
var Diff_NDVI_count=urbanNDVI.reduceRegion({geometry: feature.geometry(),  reducer: ee.Reducer.count(),  scale: 500,
});
  var nightUHI_count=nighturbanLST_AQUA.reduceRegion({geometry: feature.geometry(),  reducer: ee.Reducer.count(),  scale: 1000,
});

var Diff_NDBI_count=urbanNDBI.reduceRegion({geometry: feature.geometry(),  reducer: ee.Reducer.count(),  scale: 500,
});

return feature.set({'UHIDAY': UHI.get('mean'), 'UHINIGHT': UHINIGHT.get('mean'), 'NDVI_diff': Diff_NDVI.get('mean'),'BSA_diff': Diff_BSA.get('mean'), 'WSA_diff': Diff_WSA.get('mean'), 'NDBI_diff': Diff_NDBI.get('mean'), 'DEM': DEM_neigh.get('be75'), 'PM': PM_neigh.get('b1'), 'dayLST_Count': dayUHI_count.get('LST_Day_1km_mean'),'nightLST_Count': nightUHI_count.get('LST_Night_1km_mean'),'NDVI_Count': Diff_NDVI_count.get('nd'),'NDBI_Count': Diff_NDBI_count.get('nd')});
...
print(Final_data)
var Final_data=Final_data.select({propertySelectors: ['WSA_diff','BSA_diff','NDVI_diff', 'NDBI_diff','UHINIGHT','UHIDAY','DEM','PM','nbhd_code', 'dayLST_Count','nightLST_Count', 'NDVI_Count', 'NDBI_Count'], retainGeometry: false})
//print(Final_data)
//print(Final_data)
/* Export the feature collection for further analysis*/
Export.table.toDrive({
collection: Final_data,  description: 'Copenhagen', folder: 'UHI data for paper with Angel_v3',  fileFormat: 'CSV'
});
