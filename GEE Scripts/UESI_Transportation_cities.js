...
var city_diss = /* color: #98ff00 */ee.FeatureCollection('users/datadrivenlab/cities/cities_2019_diss');
...
function Extract_ptc(feature){
  var bounding_city=city_diss.filterBounds(feature.geometry())
  //Merge bus and rail stopes
  //var Merged=Bus.filterBounds(feature.geometry()).merge(Rail.filterBounds(feature.geometry()))
  var Merged=Bus.filterBounds(bounding_city.geometry()).merge(Rail.filterBounds(bounding_city.geometry()))
...
function dist_min(rando){
  ...
  var Bus_buffed=Bus.filterBounds(bounding_city.geometry()).map(bus_buff)
  var Rail_buffed=Rail.filterBounds(bounding_city.geometry()).map(rail_buff)
...
// // Exporting as csvs. 
Export.table.toDrive({collection: Final, description: 'Trans_cities', folder: 'Trans_Automation_UESI', fileFormat: 'CSV'})
