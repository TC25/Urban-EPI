...
var MODIS_Bands=ee.ImageCollection('MODIS/006/MYD09A1').filterDate('2016-01-01', '2016-12-31')
print(MODIS_Bands)
//Map.addLayer(LST_Day_AQUA_mean)
//For NDVI, EVI
//var Monthfilter=ee.Filter.dayOfYear(122,152)
var VI = ee.ImageCollection("MODIS/006/MYD13Q1").filterDate('2016-01-01', '2016-12-31')//.filter(Monthfilter);
var Alb = ee.ImageCollection("MODIS/006/MCD43A3").filterDate('2016-01-01', '2016-12-31')//.filter(Monthfilter);
...
function featurecreate(feature){
  var DelNDVI=NDVI_diff.reduceRegion({geometry: feature.geometry(),  reducer: ee.Reducer.mean(),  scale: 300,
});
var DelBSA=BSA_diff.reduceRegion({geometry: feature.geometry(),  reducer: ee.Reducer.mean(),  scale: 300,
});

var DelNDBI=NDBI_diff.reduceRegion({geometry: feature.geometry(),  reducer: ee.Reducer.mean(),  scale: 1000,
});
...
return feature.set({'DelNDVI': DelNDVI.get('mean'), 'DelBSA': DelBSA.get('mean'), 'DelNDBI': DelNDBI.get('mean')});
...
print(Final_data)
var Final_data=Final_data.select({propertySelectors: ['DelNDVI', 'DelBSA', 'DelNDBI','nbhd_code'], retainGeometry: false})
//print(Final_data)
//print(Final_data)
/* Export the feature collection for further analysis*/
Export.table.toDrive({
collection: Final_data,  description: 'UHI_cities_physical',  fileFormat: 'CSV'
});
